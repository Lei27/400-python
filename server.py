        ...
        if not os.path.exists(filename):  
            ...
            if lasttimestr == timestr and "Range" not in request.headers:  
                ...
                if ext.upper() in filedic or (ext == "" and "gzip" in Accept_Encoding):  
                    if not os.path.exists(cache_filename) or os.path.getmtime(cache_filename) < float(filemd5):  
                    ...
                    try:  
                        ...
                        if msg.errno == errno.EAGAIN:  
                            ...
                            if headend > 0:  
                                headlen = headend + 4  
                            data_len = len(datas)  
                            if (contentlen > 0 and headlen > 0 and (contentlen + headlen) == data_len) or (contentlen == -1 and headlen == data_len):  
